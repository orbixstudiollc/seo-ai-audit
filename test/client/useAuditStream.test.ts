 ...
 * possible at any point. Frames are built the same way the /api/audit route
 * builds them — `formatAuditEvent` — then read back with `parseAuditFrame`, so
 * this also pins producer/consumer symmetry over the wire, not just the
 * reducer's switch statement.
 */

function roundTrip(event: AuditStreamEvent): AuditStreamEvent {
  const frame = formatAuditEvent(event);
