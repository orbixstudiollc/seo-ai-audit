...
import { AgentAuditRunner } from "../../components/audit/AgentAuditRunner";
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const title = host ? `Agent audit ${host}` : "Agent audit";
  const description = host
    ? `Plan and run every relevant AI-search visibility check for ${host}.`
    : "Plan and run every relevant AI-search visibility check.";
  return {
    title,
    description,
    robots: { index: false },
...
export default async function AgentAuditPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const rawUrl = typeof params.url === "string" ? params.url : undefined;
  const url = parseUrl(rawUrl);
  ...
  return (
    <main className="flex flex-1 flex-col px-6 py-10 sm:py-14">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-2 pb-4">
        ...
        <p className="break-all font-mono text-xs text-text-3">Agent audit {url}</p>
      </div>

      <AgentAuditRunner url={url} />
    </main>
  );
}
