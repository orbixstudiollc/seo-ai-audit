...
export function LoginForm({ showGoogle, showDemoLogin, demoError }: LoginFormProps) {
  ...
  return (
    <div className="flex flex-1 items-center justify-center bg-surface-0 px-4 py-16 font-sans text-text-1">
      <div className="w-full max-w-sm">
        ...
        {showDemoLogin ? (
          <div className="mt-5 border border-accent-ink/40 bg-accent-ink/[0.06] p-5">
            <p className="font-mono text-[11px] uppercase tracking-widest text-accent-ink">
              Local demo
            </p>
            <p className="mt-2 text-sm leading-relaxed text-text-2">
              Skip signup — one click drops you into the dashboard with a
              throwaway account and mock (non-LLM) audit data.
            </p>
            {demoError ? (
              <p role="alert" className="mt-2 text-sm text-score-weak">
                Could not create the demo account. Try again.
              </p>
            ) : null}
            <a
              href="/api/demo-login"
              className="mt-4 inline-block w-full bg-accent-ink px-5 py-2.5 text-center text-sm font-semibold uppercase tracking-wide text-surface-1 transition-colors hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-ink focus-visible:ring-offset-2"
            >
              Try the demo &rarr;
            </a>
          </div>
