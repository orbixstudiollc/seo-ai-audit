...
import { HISTORY_KEY, LEGACY_HISTORY_KEY, LEGACY_HISTORY_V1_KEY, addHistoryRecord, averageScore, filterAndSortHistory, isHistoryRecord, loadHistory, type AuditHistoryRecord } from "./index";

function record(overrides: Partial<AuditHistoryRecord> = {}): AuditHistoryRecord {
  return { id: "1", version: 3, url: "https://example.com/post", title: "Example post", mode: "single", createdAt: "2026-07-19T10:00:00.000Z", status: "complete", scores: { aeo: 80, geo: 70, citability: 60, aiOverview: 50 }, ...overrides };
}

describe("local audit history", () => {
  it("accepts every query lifecycle and rejects future records", () => { for (const status of ["started", "complete", "partial", "failed"] as const) expect(isHistoryRecord(record({ status, scores: status === "started" || status === "failed" ? null : record().scores }))).toBe(true); expect(isHistoryRecord({ ...record(), version: 4 })).toBe(false); });
  ...
  it("migrates existing v2 history when v3 is absent", () => { const legacy = { ...record(), version: 2 }; const storage = { getItem: (key: string) => key === HISTORY_KEY ? null : key === LEGACY_HISTORY_KEY ? JSON.stringify([legacy]) : null }; expect(loadHistory(storage)).toEqual([record()]); });
