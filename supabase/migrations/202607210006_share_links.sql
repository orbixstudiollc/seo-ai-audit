-- Public share links: opt-in, revocable tokens that let ANYONE open a stored
-- audit report read-only from anywhere (D-021). Same posture as every
-- phase-1+ table: RLS enabled, zero public grants, server-secret access only,
-- and the account-claim RPC updated IN THE SAME migration (202607200004/5
-- precedent — shipping them apart silently breaks workspace linking).

create table if not exists public.share_links (
  -- 128-bit hex token minted server-side; the token IS the capability.
  token text primary key,
  owner_hash text not null,
  audit_id text not null,
  created_at timestamptz not null default now(),
  -- One stable link per report per owner; re-sharing returns the same token.
  unique (owner_hash, audit_id)
);

alter table public.share_links enable row level security;
revoke all on public.share_links from public, anon, authenticated;

-- Account claim: move device-owned share links to the verified account
-- workspace so shared URLs keep working after sign-in. If the account already
-- shared the same audit, keep the account's token and drop the device one.
